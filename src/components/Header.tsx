...
const Header: React.FC<HeaderProps> = ({ cartItemsCount, fixed=true }) => {
  ...
  return (
    <header className={`${scrolled ? 'shadow-md bg-white text-black' : 'bg-transparent text-white'} ${fixed ? 'fixed top-0 z-50': 'py-6 bg-[#096D71]'} transition-all duration-300 w-full`}>
      <div className="container lg:block hidden mx-auto px-4">
        <div className="flex items-center justify-between">
          ...
          {/* Centered Logo */}
          <div className={`${fixed ? 'translate-x-[40%]': 'absolute translate-x-[-25%] top-[-10px] left-[50%]'} flex items-center gap-2 rounded-full overflow-hidden aspect-square translate-y-[-10px] w-40`}>
      ...
      <div
        className="container lg:hidden flex items-center justify-between"
      >
        <div
        ...
        >
          <img src="/assets/images/logo.jpg" className="w-full h-full scale-150 translate-y-2" />
        </div>
        <div className="flex gap-6">
          <div
            className="tp-header-action-item d-flex tp-header-action-item-search"
          >
            <button
              type="button"
          ...
          <div
            className="tp-header-action-item d-flex m-0 tp-header-action-item-search"
          >
            <button
              type="button"
              className="tp-header-action-btn tp-search-open-btn"
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className='text-right'>
              <DropdownMenuItem><Link to="/" className='w-full'>الرئيسية</Link></DropdownMenuItem>
              <DropdownMenuItem><Link to="/events" className='w-full'>الفاعليات</Link></DropdownMenuItem>
              <DropdownMenuItem><Link to="/store" className='w-full'>المتجر</Link></DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem><Link to="/releases" className='w-full'>إصدارات</Link></DropdownMenuItem>
              <DropdownMenuItem><Link to="/contact" className='w-full'>اتصل بنا</Link></DropdownMenuItem>
