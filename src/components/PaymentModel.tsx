...
export default function PaymentModal({ 
    ...
    const { increaseQuantity, decreaseQuantity, removeBook, clearCart } = useBooks();
    ...
    const handleCheckout = async () => {
        ...
        try {
            const response = await fetch('https://tafra.learnock.com/api/payments/create', {
                method: 'POST',
                headers: {
            ...
            localStorage.setItem('paymentReference', data.referenceNumber);
    ...
    const totalPrice = books.reduce((sum, book) => sum + (book.price * (book.quantity || 1)), 0);
                ...
                <div className="inline-block align-bottom bg-white rounded-lg text-right overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
                    <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div className="flex justify-between items-start">
                            <h3 className="text-2xl font-bold text-gray-900">إتمام عملية الشراء</h3>
                            <button 
                                onClick={onClose}
                        ...
                        <div className="mt-6 space-y-6">
                            ...
                            <div className="border-b pb-6">
                                <h4 className="text-lg font-medium text-gray-900 mb-4">معلومات العميل</h4>
                            ...
                            <div className="space-y-4">
                                <h4 className="text-lg font-medium text-gray-900">تفاصيل الطلب</h4>
                                {books.map((book) => (
                                    <div key={book.id} className="flex justify-between items-center border-b pb-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-gray-600">{book.price} ج.م</p>
                                            </div>
                                        </div>
                                        
                                        <div className="flex items-center gap-2">
                                            <button
                                                onClick={() => decreaseQuantity(String(book.id))}
                                            ...
                                            <button
                                                onClick={() => increaseQuantity(String(book.id))}
                                                className="w-8 h-8 flex items-center justify-center border border-gray-300 rounded-md text-gray-600 hover:bg-gray-100"
                                            >
                                                +
                                            </button>
                                            <button
                                                onClick={() => removeBook(String(book.id))}
